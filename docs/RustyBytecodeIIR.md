...
Please checkout the module [`rustybytecode::ir`](https://docs.rs/rustybytecode/latest/rustybytecode/ir/index.html) for more information.

To generate RustybytecodeIR from a method, use the following code.

```rust
use rustybytecode:jvm::Method;
use rustybytecode::ir::{RustybytecodeIRMethod, RustybytecodeIRMethodExt};

fn rustybytecode_ir(method: &Method) -> Result<RustybytecodeIRMethod, Box<dyn std::error::Error>> {
    let rustybytecode_ir_method = method.brew()?;
    Ok(rustybytecode_ir_method)
}
```

The following is an example of the generated IR from the method `test()` in [TestAnalysis.java](/test_data/rustybytecode/org/rustybytecode/test/TestAnalysis.java).

You may notice that there are lots of `nop`s in the generated RustybytecodeIR.
This because we indent to maintain a bijection between the original bytecode and the generated RustybytecodeIR.
Such a bijection facilitates the analysis involving dynamic execution - the runtime information (e.g., coverage) can be applied to RustybytecodeIR without needing any remapping.

| Address | JVM Instruction  | RustybytecodeIR                                                                                |
| :------ | :--------------- | :------------------------------------------------------------------------------------ |
...
| `#0015` | `iload`          | `nop`                                                                                 |
| `#0017` | `iload_2`        | `nop`                                                                                 |
| `#0018` | `invokevirtual`  | `%24 = call int %this@org/rustybytecode/test/TestAnalysis::callMe(%0, Phi(%3, %16), %arg1)` |
| `#001B` | `istore`         | `nop`                                                                                 |
| `#001D` | `iload`          | `nop`                                                                                 |
...
| `#003C` | `invokevirtual`  | `%60 = call int %this@org/rustybytecode/test/TestAnalysis::callMe(%56, %58, %59)`           |
| `#003F` | `pop`            | `nop`                                                                                 |
...
| `#00A8` | `invokevirtual`  | `%168 = call int %this@org/rustybytecode/test/TestAnalysis::callMe(%165, %166, %167)`       |
| `#00AB` | `pop`            | `nop`                                                                                 |
| `#00AC` | `goto`           | `goto #00BA`                                                                          |
| `#00AF` | `aload_0`        | `nop`                                                                                 |
...
| `#00B3` | `invokevirtual`  | `%179 = call int %this@org/rustybytecode/test/TestAnalysis::callMe(%176, %177, %178)`       |
| `#00B6` | `pop`            | `nop`                                                                                 |
| `#00B7` | `goto`           | `goto #00BA`                                                                          |
| `#00BA` | `iload`          | `nop`                                                                                 |
