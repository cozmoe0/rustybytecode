...
use super::{Identifier, Operand, RustybytecodeIRMethod, RustybytecodeInstruction};

/// An error that occurs when generating Rustybytecode IR.
#[derive(Debug, thiserror::Error)]
pub enum RustybytecodeIRBrewingError {
...
struct RustybytecodeIRGenerator<'m> {
    ir_instructions: BTreeMap<ProgramCounter, RustybytecodeInstruction>,
    method: &'m Method,
    body: &'m MethodBody,
    control_flow_edges: BTreeMap<(ProgramCounter, ProgramCounter), ControlTransfer>,
}

impl Analyzer for RustybytecodeIRGenerator<'_> {
    type Location = ProgramCounter;
    type Fact = JvmStackFrame;
    type Err = RustybytecodeIRBrewingError;
    ...
    fn entry_fact(&self) -> Result<Self::AffectedLocations, Self::Err> {
        let first_pc = self
            ...
            .instructions
            .entry_point()
            .ok_or(RustybytecodeIRBrewingError::MalformedControlFlow)?
            .0
            .to_owned();
        JvmStackFrame::new(
            self.method
    ...
    fn analyze_location(
        ...
        let edges_and_frames = match &ir_instruction {
            RustybytecodeInstruction::Nop => {
                let next_pc = self.next_pc_of(location)?;
                let edge = (location, next_pc, Unconditional);
                vec![(edge, frame)]
            }
            RustybytecodeInstruction::Return(_) => Vec::default(),
            RustybytecodeInstruction::Definition {
            ...
            RustybytecodeInstruction::Jump { condition, target } => {
            ...
            RustybytecodeInstruction::Switch {
                default,
                branches,
                match_value,
            } => {
            ...
            RustybytecodeInstruction::SubroutineRet(_) => mem::take(&mut frame.possible_ret_addresses)
                .into_iter()
                .map(|return_address| {
    ...
    fn merge_facts(
        ...
        current_fact
            .merge(incoming_fact)
            .map_err(RustybytecodeIRBrewingError::MergeError)
    }
}

impl<'m> RustybytecodeIRGenerator<'m> {
    fn next_pc_of(&self, pc: ProgramCounter) -> Result<ProgramCounter, RustybytecodeIRBrewingError> {
        self.body
            .instructions
            .next_pc_of(&pc)
            .ok_or(RustybytecodeIRBrewingError::MalformedControlFlow)
    ...
    fn for_method(method: &'m Method) -> Result<Self, <Self as Analyzer>::Err> {
        let body = method
            .body
            .as_ref()
            .ok_or(RustybytecodeIRBrewingError::NoMethodBody)?;
        Ok(Self {
...
/// An extension trait for [`Method`] that generates Rustybytecode IR.
pub trait RustybytecodeIRMethodExt {
    /// Generates Rustybytecode IR for the method.
    /// # Errors
    /// See [`RustybytecodeIRBrewingError`] for more information.
    fn brew(&self) -> Result<RustybytecodeIRMethod, RustybytecodeIRBrewingError>;
}

impl RustybytecodeIRMethodExt for Method {
    fn brew(&self) -> Result<RustybytecodeIRMethod, RustybytecodeIRBrewingError> {
        let (instructions, control_flow_graph) = RustybytecodeIRGenerator::for_method(self)?.generate()?;
        Ok(RustybytecodeIRMethod {
...
impl RustybytecodeIRGenerator<'_> {
    fn generate(
        mut self,
    ) -> Result<
        (
            InstructionList<RustybytecodeInstruction>,
            ControlFlowGraph<(), ControlTransfer>,
        ),
        RustybytecodeIRBrewingError,
    > {
        self.analyze()?;
