...
mod test {
    ...
    fn last_instruction() {
        let instruction_list = InstructionList::from([
            (0.into(), RustybytecodeInstruction::Nop),
            (
                1.into(),
                RustybytecodeInstruction::Jump {
                    condition: None,
                    target: 1.into(),
                },
            ),
            (2.into(), RustybytecodeInstruction::Return(None)),
        ]);
        assert_eq!(
            Some((&2.into(), &RustybytecodeInstruction::Return(None))),
