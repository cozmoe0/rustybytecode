...
use rustybytecode::{
    ir::{
        expression::Expression, DefUseChain, Identifier, LocalValue, Operand,
        RustybytecodeIRMethodExt, RustybytecodeInstruction,
    },
...
fn get_test_class() -> Class {
    let bytes = include_bytes!(concat!(
        env!("OUT_DIR"),
        "/rustybytecode/java_classes/org/rustybytecode/test/TestAnalysis.class"
    ));
...
fn brew_ir() {
    ...
    assert!(matches!(
        ...
        RustybytecodeInstruction::Return(Some(Operand::Just(Identifier::Arg(1))))
    ));
}
...
proptest! {
    ...
    fn du_chain_defs(local_idx in any::<u16>()) {
        ...
        if let Some(RustybytecodeInstruction::Definition { .. }) = ir_method.instructions.get(&pc) {
...
fn cfg_to_dot() {
    ...
    use rustybytecode::ir::control_flow::ControlTransfer;
...
#[cfg(feature = "petgraph")]
fn dominance() {
    use rustybytecode::jvm::code::ProgramCounter;
