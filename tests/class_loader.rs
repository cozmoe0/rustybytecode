...
fn create_test_dir_class_path() -> DirectoryClassPath {
    DirectoryClassPath::new(concat!(env!("OUT_DIR"), "/rustybytecode/java_classes"))
}
...
fn load_class() {
    ...
    let class = class_loader.load_class("org/rustybytecode/test/MyClass").unwrap();
    assert_eq!(class.binary_name, "org/rustybytecode/test/MyClass");
}
...
impl ClassPath for MockClassPath<'_> {
    fn find_class(&self, _binary_name: &str) -> Result<Class, Error> {
        ...
        let reader = test_data_class!("rustybytecode", "org/rustybytecode/test/MyClass");
